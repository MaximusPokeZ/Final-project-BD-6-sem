...
import org.example.impl.ServiceEnrichment;
...
class ServiceTest {
    ...
    void testMongoReadWriteValues() {
        ...
        var testDocument = new Document()
                ...
                .append("fieldValue", "testFieldValue")
    ...
    @Test
    void testServiceEnrichmentOneRuleSpecific() {
        try {
            createAndCheckRuleInPostgreSQL(
                    ENRICHMENT_ID,
                    1L,
            ...
            var listDataIn = List.of(
                    TestDataSpecific.builder().model("Ivolga").coordX1(18.12).coordX2(89.34).state(null).build(),
                    TestDataSpecific.builder().model("Lastochka").coordX1(18.12).coordX2(18.12).state(null).build()
            );
            ...
            var tmp = List.of(
                    TestDataSpecific.builder().model("Ivolga").coordX1(18.12).coordX2(89.34).state("active").build(),
                    TestDataSpecific.builder().model("Lastochka").coordX1(18.12).coordX2(18.12).state("non-active").build()
            );

            var listExpectedJson = tmp.stream().map(data -> toJsonNode(toJson(data))).toList();
    ...
    @Test
    void testServiceEnrichmentTwoRuleSpecific() {
        try {
            createAndCheckRuleInPostgreSQL(
                    ENRICHMENT_ID,
                    1L,
    ...
    private Config replaceConfigForTest(Config config) {
        ...
        return config.withValue("kafka.consumer.bootstrap.servers", ConfigValueFactory.fromAnyRef(kafka.getBootstrapServers()))
                ...
                .withValue("kafka.consumer.topics", ConfigValueFactory.fromAnyRef(List.of(TEST_TOPIC_IN)))
                .withValue("kafka.producer.topic", ConfigValueFactory.fromAnyRef(TEST_TOPIC_OUT));
    }
